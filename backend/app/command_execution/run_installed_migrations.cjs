...
if (!packageName || !fromVersion || !toVersion) {
  throw new Error('usage: node run_installed_migrations.cjs <package> <from> <to>');
}
...
function migrationVersion(value, name) {
  const normalized = semver.valid(value) || semver.coerce(value);
  if (!normalized) {
    throw new Error(`Invalid migration version for ${packageName}:${name}: ${value}`);
  }
  return normalized;
}
...
const migrations = Object.entries(collection.schematics || {})
  ...
  .map(([name, value]) => [name, value, migrationVersion(value.version, name)])
...
(async () => {
  process.stdout.write(`MIGRATION_COLLECTION ${packageName} ${collectionPath}\n`);
