...
export function usePlanReview({ runId, stateVersion, workflowEvents, planningJob, connectionStatus, refreshAuthoritativeState }: { runId: string; stateVersion: number; workflowEvents: Array<{ event_type: string; sequence: number }>; planningJob?: PlanningJobProjection | null; connectionStatus: string; refreshAuthoritativeState: () => Promise<unknown> }) {
  ...
  const mutate = useCallback(async <T,>(action: string, operation: () => Promise<T>) => { if (mutating.current) return null; mutating.current = true; setLastAction(action); setStatus("running"); setError(null); try { const result = await operation(); await refreshAuthoritativeState(); await refresh(); return result; } catch (reason) { if (reason instanceof ApiClientError && reason.status === 409) { setStatus("stale"); await refresh(); await refreshAuthoritativeState(); } else if (reason instanceof ApiClientError && [401, 403].includes(reason.status)) setStatus("authorization"); else { setStatus("failure"); setError(`${action} failed. Correlation ID: ${correlationId(reason)}`); } return null; } finally { mutating.current = false; } }, [refresh, refreshAuthoritativeState]);
