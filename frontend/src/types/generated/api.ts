...
export type TimingMeasurementStatus = "complete" | "partial" | "unavailable";
export type TimingSpanStatus = "not_started" | "running" | "completed" | "unavailable";
export type TimingActivityDto = { duration_seconds: number | null; measured_count: number; unmeasured_count: number; active_count: number; measurement_status: TimingMeasurementStatus };
export type TimingSpanDto = { key: string; label: string; status: TimingSpanStatus; started_at: string | null; finished_at: string | null; duration_seconds: number | null };
export type RunTimingActivityDto = { llm: TimingActivityDto; commands: TimingActivityDto; human_approval_wait: TimingActivityDto; repair: TimingActivityDto; validation: TimingActivityDto; sealing: TimingActivityDto };
export type RunTimingDto = { run_id: string; status: RunStatus; as_of: string; started_at: string | null; finished_at: string | null; total_duration_seconds: number | null; total_measurement_status: "running" | "complete" | "unavailable"; activity: RunTimingActivityDto; phases: TimingSpanDto[]; stages: TimingSpanDto[] };
...
export type PlanningJobProjectionDto = { id: string; status: string; current_step: string; attempt: number; max_attempts: number; retryable?: boolean | null; next_attempt_at?: string | null; last_error_code?: string | null; last_error_message?: string | null; last_error_stage?: string | null; correlation_id?: string | null; updated_at: string };
