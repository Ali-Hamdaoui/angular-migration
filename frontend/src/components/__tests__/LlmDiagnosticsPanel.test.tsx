...
const invocation = { invocation_id: "llm-1", run_id: "run-1", status: "completed" as const, role: "phase_proposer", task_type: "assistant_response", provider: "azure_openai", deployment_alias: "azure-openai", artifact_ids: ["artifact-usage"], artifact_checksums: { "artifact-usage": "sha256:usage" }, input_tokens: 10, output_tokens: 5, total_tokens: 15, input_cost_usd: 0.00001, output_cost_usd: 0.00002, total_cost_usd: 0.00003, retries: 1, latency_ms: 250, failure_code: null, state_version: 3, event_sequence: 4, idempotent_replay: false };

describe("LlmDiagnosticsPanel", () => {
  beforeEach(() => {
    vi.mocked(getLlmReadiness).mockResolvedValue({ status: "ready", provider: "azure_openai", deployment_configured: true, model_capability: "responses_json_object", error_code: null });
  ...
  it("renders provenance, token cost, and invokes through the typed backend contract", async () => {
    ...
    await waitFor(() => expect(invokeLlmSmoke).toHaveBeenCalledWith(expect.objectContaining({ run_id: "run-1", expected_state_version: 2, actor: "control-tower" })));
  });
