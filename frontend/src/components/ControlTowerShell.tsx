...
export function ControlTowerShell({ run, runId, connectionStatus }: { run: MigrationRun; runId?: string; connectionStatus?: ConnectionStatus }) {
  ...
  return <main className={styles.shell}><RunHeader run={run} /><WorkflowTimeline run={run} /><StageCards run={run} /><div className={styles.twoColumns}><AgentActivityPanel run={run} /><ValidationGatePanel run={run} />{runId ? <><BaselineValidationPanel runId={runId} stateVersion={stateVersion} connectionStatus={matrixConnection} /><BaselineQualificationPanel runId={runId} stateVersion={stateVersion} /></> : null}<ApprovalPanel run={run} /><ArtifactPanel run={run} /><AssistantPanel /><LlmUsagePanel run={run} /><DiagnosticsPanel run={run} /><ReportPanel run={run} /></div></main>;
}
