...
const response: PlanResponse = {
  ...
  stage_plan: { stage_plan_id: "stage-plan-1", stage_id: "stage-18-to-19", plan_version: 1, input_fingerprint: checksum("c"), evidence_set_checksum: checksum("g"), input_workspace_fingerprint: checksum("h"), source_family: "angular-18.x", source_exact: "18.2.13", target_family: "angular-19.x", target_exact: "19.2.0", execution_profile_id: "profile-1", commands: { bootstrap_install: [{ command_id: "npm-ci", template_id: "npm-ci-bootstrap", template_version: 2, parameter_bindings: { package_manager: "npm" }, executable: "npm", arguments: ["ci"], shell: false, working_directory_alias: "STAGE_WORKSPACE_18_TO_19", timeout_seconds: 300, network_profile: "approved-registries-only", runtime_profile_checksum: checksum("i"), cancellation_policy: "terminate_process_tree", conditional: false }] }, build_system_decision: { decision_id: "decision-1", builder: "@angular-devkit/build-angular:application", action: "preserve", rationale: "Keep builder", checksum: checksum("d") }, validation_policy: { policy_id: "validation-v1", baseline_comparison_required: true, route_comparison_required: true, backend_comparison_required: true, required_checks: ["build", "test"] }, recovery_policy: { policy_id: "recovery-v1", safe_boundaries: ["before-install"], rerun_read_only_steps: true, reconstruct_mutating_steps: true }, repair_policy: { policy_id: "repair-v1", enabled: true, proposer_reviewer_required: true, human_apply_required: true }, forbidden_change_policy: { policy_id: "forbidden-v1", actions: ["optional_signals_migration"] }, checksum: checksum("e") }, plan_checksum: checksum("b"), stage_plan_checksum: checksum("e"), artifact_ids: ["plan-artifact"], artifact_checksums: { "plan-artifact": checksum("f") }, artifact_links: { "plan-artifact": "/api/v1/artifacts/plan-artifact" }, builder_decision: {}, state_version: 3, event_sequence: 2, idempotent_replay: false,
};
const state = { run_id: "run-1", status: "PLANNING_RUNNING", state_version: 1, artifacts: [artifact], planning_job: { id: "job-1", status: "generating_plan", current_step: "generating_plan", attempt: 1, max_attempts: 3, retryable: false, updated_at: "now" } } as unknown as AuthoritativeRunStateDto;
...
describe("MigrationPlanPanel", () => {
  ...
  it("renders distinct evidence bindings and versioned structured command fields", async () => {
    ...
    expect(screen.getByText("Evidence-set checksum")).toBeInTheDocument();
    expect(screen.getByText("Physical workspace fingerprint")).toBeInTheDocument();
    expect(screen.getByText("npm-ci-bootstrap v2")).toBeInTheDocument();
    expect(screen.getByText("STAGE_WORKSPACE_18_TO_19")).toBeInTheDocument();
    expect(screen.getByText("terminate_process_tree")).toBeInTheDocument();
    fireEvent.click(screen.getByRole("tab", { name: "Artifacts" }));
    expect(screen.getByRole("link", { name: "plan-artifact" })).toHaveAttribute("href", "/api/v1/artifacts/plan-artifact");
  });

  it("shows backend-owned plan generation and never exposes a manual generate button", async () => {
    vi.mocked(getPlan).mockRejectedValue(new ApiClientError("missing", 404));
    render(<MigrationPlanPanel {...props} />);
    expect(await screen.findByText("The backend is generating the migration plan.")).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: /Generate MigrationPlan/i })).not.toBeInTheDocument();
  });

  it("shows the exact post-G06 boundary", async () => {
    vi.mocked(getPlan).mockResolvedValue(response);
    render(<MigrationPlanPanel {...props} initialState={{ ...state, status: "WAITING_STAGE_PREPARATION", planning_job: { ...state.planning_job!, status: "completed", current_step: "completed" } } as AuthoritativeRunStateDto} />);
    expect(await screen.findByText("Plan approved for execution. Waiting for authoritative stage preparation.")).toBeInTheDocument();
  });
