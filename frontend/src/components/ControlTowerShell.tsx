...
export function ControlTowerShell({ run, runId, connectionStatus }: { run: MigrationRun; runId?: string; connectionStatus?: ConnectionStatus }) {
  ...
  return <main className={styles.shell}><div className={styles.content}><RunHeader run={run} /><WorkflowTimeline run={run} /><StageCards run={run} /><div className={styles.dashboardGrid}><div className={styles.primaryColumn}><AgentActivityPanel run={run} />{runId ? <><BaselineValidationPanel runId={runId} stateVersion={stateVersion} connectionStatus={matrixConnection} /><BaselineQualificationPanel runId={runId} stateVersion={stateVersion} /></> : null}<ArtifactPanel run={run} /><ReportPanel run={run} /></div><aside className={styles.secondaryColumn}><ValidationGatePanel run={run} /><ApprovalPanel run={run} /><DiagnosticsPanel run={run} /><LlmUsagePanel run={run} />{runId ? <AssistantPanel runId={runId} phase={String(run.run_phase)} stateVersion={stateVersion} workflowStatus={String(run.status)} /> : null}</aside></div></div></main>;
}
