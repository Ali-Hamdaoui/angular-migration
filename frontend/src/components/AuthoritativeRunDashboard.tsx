...
<section hidden={activeSection !== "pipeline"} className="controlTowerSection" aria-labelledby="pipeline-navigation-item">{heading("Pipeline", "A compact stage view of the authoritative workflow.")}<PipelineSection state={state} retryError={retryError} retrying={retryingSourceIntake} onRetry={() => void retrySourceIntake()} qualificationAvailable={qualificationAvailable} qualificationActionRequired={qualificationActionRequired} g02ActionRequired={g02ActionRequired} focusStage={focusStage}>{(selectedStage) => <><div hidden={selectedStage !== "Source review & G02"}><SourceSnapshotPanel {...shared} /><G02ReviewPanel {...shared} /></div><div hidden={selectedStage !== "Baseline qualification"} aria-label="G03 review"><BaselineQualificationPanel runId={runId} stateVersion={state.state_version} workflowEvents={state.workflow_events} refreshAuthoritativeState={refresh} /></div></>}</PipelineSection><div hidden>{has("EXECUTION_PROFILE_RESOLUTION_STARTED", "EXECUTION_PROFILE_RESOLVED", "EXECUTION_PROFILE_SELECTED", "EXECUTION_PROFILE_BLOCKED") ? <ExecutionProfilePanel {...shared} /> : null}{has("BASELINE_WORKSPACE_STARTED", "BASELINE_WORKSPACE_READY") ? <BaselinePreparationPanel {...shared} /> : null}{baselineAvailable ? <BaselineInstallationPanel {...shared} connectionStatus={status} /> : null}{baselineAvailable ? <BaselineValidationPanel runId={runId} stateVersion={state.state_version} connectionStatus={status} availableKinds={(["build", "test", "lint"] as const)} /> : null}{has("G06_APPROVED", "STAGE_PLAN_CREATED", "EXECUTION_PROFILE_SELECTED", "BASELINE_WORKSPACE_READY") ? <CommandPolicyInspector runId={runId} runState={state} stateVersion={state.state_version} connectionStatus={status} workflowEvents={state.workflow_events} refreshAuthoritativeState={refresh} /> : null}</div></section>
        ...
        <section hidden={activeSection !== "feasibility"} className="controlTowerSection" aria-labelledby="feasibility-navigation-item">{heading("Feasibility & G05", "Compatibility evidence and feasibility approval.")}{state.planning_job || has("G04_APPROVED", "COMPATIBILITY_RESOLUTION_STARTED", "COMPATIBILITY_RESOLUTION_COMPLETED", "COMPATIBILITY_RESOLUTION_BLOCKED", "PLANNING_INPUT_RESOLUTION_FAILED", "PLANNING_RETRY_SCHEDULED", "PLANNING_FAILED", "G05_CREATED") ? <FeasibilityPanel {...shared} connectionStatus={status} artifacts={state.artifacts} workflowEvents={state.workflow_events} refreshAuthoritativeState={refresh} /> : available("Feasibility")}</section>
        <section hidden={activeSection !== "planning"} className="controlTowerSection" aria-labelledby="planning-navigation-item">{heading("Planning & G06", "Migration plan, review, and approval.")}{state.planning_job || has("G05_APPROVED", "MIGRATION_PLAN_CREATED", "STAGE_PLAN_CREATED", "PLAN_REVISION_CREATED", "PLANNING_INPUT_RESOLUTION_FAILED", "PLANNING_RETRY_SCHEDULED", "PLANNING_FAILED", "G06_CREATED") ? <><MigrationPlanPanel {...shared} connectionStatus={status} artifacts={state.artifacts} workflowEvents={state.workflow_events} refreshAuthoritativeState={refresh} /><PlanReviewPanel {...shared} connectionStatus={status} refreshAuthoritativeState={refresh} /></> : available("Planning")}</section>
