  ...
  const pipelineStageContent = useMemo(() => {
    ...
    return workspace.journey.map((milestone): PipelineStageContent => {
      ...
      const summaryPanel = (
        isCurrentTransformation ? (
          ...
          {milestone.key === "baseline" && !hasGatePackage ? (
            <>
              {hasEvent(state, "EXECUTION_PROFILE_RESOLUTION_STARTED", "EXECUTION_PROFILE_RESOLVED", "EXECUTION_PROFILE_SELECTED", "EXECUTION_PROFILE_BLOCKED") ? <ExecutionProfilePanel runId={runId} initialState={state} /> : null}
              {hasEvent(state, "BASELINE_WORKSPACE_STARTED", "BASELINE_WORKSPACE_READY") ? <BaselinePreparationPanel runId={runId} initialState={state} /> : null}
              {hasEvent(state, "BASELINE_WORKSPACE_STARTED", "BASELINE_WORKSPACE_READY") ? <PanelBoundary title="Baseline installation"><BaselineInstallationPanel runId={runId} initialState={state} connectionStatus={status} refreshAuthoritativeState={refresh} /></PanelBoundary> : null}
              {hasEvent(state, "BASELINE_WORKSPACE_STARTED", "BASELINE_WORKSPACE_READY") ? <BaselineValidationPanel runId={runId} stateVersion={state.state_version} connectionStatus={status} availableKinds={["build", "test", "lint"]} /> : null}
          ...
          {milestone.key === "discovery" && !hasGatePackage ? (
            <>
              {hasEvent(state, "DISCOVERY_STARTED", "SCANNER_COMPLETED", "DISCOVERY_COMPLETED", "DISCOVERY_BLOCKED") ? <DiscoveryFindingsPanel runId={runId} stateVersion={state.state_version} connectionStatus={status} artifacts={state.artifacts} /> : null}
              {hasEvent(state, "PARITY_BASELINE_STARTED", "PARITY_BASELINE_COMPLETED", "PARITY_BASELINE_BLOCKED") ? <ParityBaselinePanel runId={runId} stateVersion={state.state_version} connectionStatus={status} artifacts={state.artifacts} /> : null}
          ...
          {milestone.key === "plan" && !hasGatePackage ? (
            <>
              ...
              {hasEvent(state, "G06_APPROVED", "STAGE_PLAN_CREATED", "EXECUTION_PROFILE_SELECTED", "BASELINE_WORKSPACE_READY") ? <CommandPolicyInspector runId={runId} runState={state} stateVersion={state.state_version} connectionStatus={status} workflowEvents={state.workflow_events} refreshAuthoritativeState={refresh} /> : null}
            </>
          ) : null}
        </div>
      );
      ...
      if (hasGatePackage) {
        ...
        else if (milestone.key === "plan") panel = <><MigrationPlanPanel runId={runId} initialState={state} connectionStatus={status} artifacts={state.artifacts} workflowEvents={state.workflow_events} refreshAuthoritativeState={refresh} headingLevel={4} /><PanelBoundary title="Planning review"><PlanReviewPanel runId={runId} initialState={state} connectionStatus={status} refreshAuthoritativeState={refresh} headingLevel={4} /></PanelBoundary></>;
