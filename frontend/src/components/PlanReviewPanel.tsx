...
export function PlanReviewPanel({ runId, initialState, connectionStatus, refreshAuthoritativeState }: { runId: string; initialState: AuthoritativeRunStateDto; connectionStatus: string; refreshAuthoritativeState: () => Promise<unknown> }) {
  ...
  return <section className={styles.panel} aria-labelledby="plan-review-title">
    <div className={styles.previewHeader}><div><p className={styles.kicker}>Plan approval · Gate G06</p><h2 id="plan-review-title">Plan approval</h2><p className={styles.note}>Executable plan data and Planning Agent interpretation are separate evidence.</p></div><span className={styles.status}>{status === "running" ? `${lastAction ?? "Working"}…` : status}</span></div>
    ...
    {review ? <>
      ...
      <div><h3>Registered evidence</h3>{artifactIds.length > 0 ? <ul className={styles.list}>{artifactIds.map((id) => <li key={id}><a href={review.artifact_links?.[id] ?? `/api/v1/artifacts/${encodeURIComponent(id)}`} target="_blank" rel="noreferrer">{id}</a> <code>{review.artifact_checksums?.[id] ?? ""}</code></li>)}</ul> : <p className={styles.note}>No reviewer artifacts are available.</p>}</div>
