...
try {
    ...
    if (-not $pythonCommand) {
        throw "Python was not found on PATH. The backend environment must provide Python."
    }

    if ($pythonCommand.Name -eq "py.exe" -or $pythonCommand.Name -eq "py") {
        & $pythonCommand.Source -3 $tempPy $db $RunId $tempJson
    }
    else {
        & $pythonCommand.Source $tempPy $db $RunId $tempJson
    }
