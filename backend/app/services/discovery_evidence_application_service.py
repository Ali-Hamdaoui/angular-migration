...
class DiscoveryEvidenceApplicationService:
    ...
    def capture(self, run_id, request):
        ...
        with self.scope() as session:
            ...
            ids, checks = [], {}
            for draft in drafts:
                artifact = store.write_text_artifact(run_id, "02_analysis/" + draft.name, draft.content, ArtifactType.JSON, created_by="discovery-evidence", created_at=self.now(), policy_version="discovery-v1")
    ...
    def block(self, run_id, request, checksum, message):
        with self.scope() as session:
            run = session.get(MigrationRunModel, run_id)
            event = self.transition(session, run, request, WorkflowEventType.DISCOVERY_BLOCKED, "discovery dependency failed", {"error_code": "DISCOVERY_DEPENDENCY_FAILED"})
    ...
    def transition(self, session, run, request, event, reason, payload):
        return StateTransitionService(session).apply_transition(TransitionRequest(run_id=run.id, expected_state_version=run.state_version, idempotency_key=request.idempotency_key + ":" + event.value + ":" + str(payload.get("scanner", "")), event_type=event, actor=request.actor, reason=reason, occurred_at=self.now(), payload=payload))
