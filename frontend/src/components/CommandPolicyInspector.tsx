...
function errorDetails(error: unknown) {
  ...
  try {
    const payload = JSON.parse(error.responseBody ?? "{}") as { error_code?: string; message?: string; correlation_id?: string; detail?: { error_code?: string; code?: string; message?: string; correlation_id?: string } };
    const detail = payload.detail ?? {};
    return { code: payload.error_code ?? detail.error_code ?? detail.code ?? (error.status === 409 ? "CONFLICT" : "BACKEND_UNAVAILABLE"), correlationId: payload.correlation_id ?? detail.correlation_id ?? null, message: payload.message ?? detail.message ?? error.message };
  } catch {
