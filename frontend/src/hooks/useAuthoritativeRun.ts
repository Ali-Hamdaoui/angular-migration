...
export function useAuthoritativeRun(runId: string, initialState: AuthoritativeRunStateDto) {
  ...
  const lastSequenceRef = useRef(Math.max(0, ...initialState.workflow_events.map((event) => event.sequence)));
  ...
  const refresh = useCallback(() => {
    ...
    return getAuthoritativeRunState(runId)
      .then((next) => {
        ...
        lastSequenceRef.current = Math.max(0, ...next.workflow_events.map((event) => event.sequence));
        seenEventIdsRef.current = new Set(next.workflow_events.map((event) => event.event_id));
        setError(null);
        setStatus("open");
      })
      .catch(() => {
  ...
  useEffect(() => {
    ...
    const onEvent = (event: MessageEvent) => {
      try {
        ...
        setState((current) => {
          if (seenEventIdsRef.current.has(next.event_id)) return current;
          if (lastSequenceRef.current > 0 && next.sequence > lastSequenceRef.current + 1) {
            setStatus("recovering");
            void refresh();
            return current;
          }
