...
const humanStatus = (value: string | null | undefined) => value ? ({ pending: "Awaiting approval", approved: "Approved", approved_with_comment: "Approved with comment", request_modification: "Changes requested", rejected: "Rejected", stale: "Needs refresh", blocked: "Blocked" } as Record<string, string>)[value] ?? value.replaceAll("_", " ") : "Not available";
const compactId = (value: string) => value.length > 18 ? `${value.slice(0, 10)}…${value.slice(-6)}` : value;
...
export function PlanReviewPanel({ runId, initialState, connectionStatus, refreshAuthoritativeState, headingLevel = 2 }: { runId: string; initialState: AuthoritativeRunStateDto; connectionStatus: string; refreshAuthoritativeState: () => Promise<unknown>; headingLevel?: PanelHeadingLevel }) {
  ...
  return <section className={styles.panel} aria-labelledby="plan-review-title">
    <div className={styles.previewHeader}><div><p className={styles.kicker}>Migration plan</p><Heading id="plan-review-title">Migration plan approval</Heading><p className={styles.note}>Review the route, risks, validation expectations, and the executable plan before approval.</p></div><span className={styles.status}>{status === "running" ? `${lastAction ?? "Working"}…` : humanStatus(status)}</span></div>
    {status === "reconnecting" ? <p role="status">Connection interrupted. Reloading the authoritative review after reconnect.</p> : null}{status === "loading" ? <p role="status">Loading authoritative plan review…</p> : null}{status === "empty" ? <p className={styles.note}>No persisted plan review is available yet. Generate the MigrationPlan first.</p> : null}{status === "authorization" ? <p role="alert">You are not authorized to review this migration plan.</p> : null}    {status === "blocked" ? <p role="alert">Plan review is blocked by missing or invalid evidence. The migration plan approval cannot advance.</p> : null}{status === "stale" ? <p className={panelStyles.stale} role="alert">This review is stale. The authoritative snapshot was refreshed; select the current version before retrying.</p> : null}{error ? <p role="alert">{error}</p> : null}
    {review ? <>
      <div className={panelStyles.grid}><div><span>Plan version</span><strong>{planVersion ?? "—"}</strong></div><div><span>Approval</span><strong>{humanStatus(review.gate_status)}</strong></div></div>
      <TechnicalDetails title="Technical plan bindings"><dl className={styles.metadataGrid}><div><dt>Plan checksum</dt><dd><code>{review.plan_checksum ?? "—"}</code></dd></div><div><dt>Stage checksum</dt><dd><code>{review.stage_plan_checksum ?? "—"}</code></dd></div></dl></TechnicalDetails>
      ...
      <div><Subheading>Migration plan decision</Subheading><p className={styles.note}>Gate decisions are backend-authoritative and checksum-bound.</p><label className={panelStyles.field}>Comment<textarea value={comment} onChange={(event) => setComment(event.target.value)} maxLength={4000} /></label><div className={panelStyles.actions}><button className={panelStyles.action} type="button" onClick={() => void submitDecision("approve")} disabled={gateBlocked || status === "running"}>Approve migration plan</button><button className={panelStyles.action} type="button" onClick={() => void submitDecision("approve_with_comment")} disabled={gateBlocked || status === "running"}>Approve with comment</button><button className={panelStyles.action} type="button" onClick={() => void submitDecision("request_modification")} disabled={gateBlocked || status === "running"}>Request plan changes</button><button className={panelStyles.action} type="button" onClick={() => void submitDecision("reject")} disabled={gateBlocked || status === "running"}>Reject plan</button></div></div>
      <div><Subheading>Registered evidence</Subheading>{artifactIds.length > 0 ? <ul className={styles.list}>{artifactIds.map((id, index) => <li key={id}><a href={review.artifact_links?.[id] ?? `/api/v1/artifacts/${encodeURIComponent(id)}`} target="_blank" rel="noreferrer" title={id}>Evidence item {index + 1}</a> <code title={id}>{compactId(id)}</code></li>)}</ul> : <p className={styles.note}>No reviewer artifacts are available.</p>}</div>
