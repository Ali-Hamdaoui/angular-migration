...
def _prepare_stage(run_id: str, request: StagePrepareRequest, actor: str, service: StagePreparationApplicationService):
    try:
        return service.prepare_stage(run_id, request.model_copy(update={"actor": actor}))
...
def prepare_stage(run_id: str, request: StagePrepareRequest, actor: str = Depends(required_authenticated_actor), service: StagePreparationApplicationService = Depends(get_stage_service)):
    return _prepare_stage(run_id, request, actor, service)


@router.post("/{run_id}/stages/{stage_id}/prepare", response_model=StagePrepareResponse)
def prepare_stage_by_id(run_id: str, stage_id: str, request: StagePrepareRequest, actor: str = Depends(required_authenticated_actor), service: StagePreparationApplicationService = Depends(get_stage_service)):
    if request.stage_key != stage_id:
        raise HTTPException(status_code=400, detail="stage_id in path does not match stage_key in body")
    return _prepare_stage(run_id, request, actor, service)


@router.post("/{run_id}/stages/{stage_id}/sandbox", response_model=StageSandboxResponse)
def create_sandbox(run_id: str, stage_id: str, request: StageSandboxRequest, actor: str = Depends(required_authenticated_actor), service: StagePreparationApplicationService = Depends(get_stage_service)):
    try:
        return service.create_sandbox(run_id, stage_id, request.model_copy(update={"actor": actor}))
    except StageApplicationError as e:
        raise HTTPException(status_code=e.status_code, detail={"error_code": e.code, "message": e.message}) from e


@router.get("/{run_id}/approvals/G07", response_model=G07ReviewResponse)
def inspect_g07(run_id: str, stage_id: str | None = None, actor: str = Depends(required_authenticated_actor), service: StagePreparationApplicationService = Depends(get_stage_service)):
    ...
    try:
        result = service.get_g07(run_id, stage_id, actor)
    except StageApplicationError as e:
        raise HTTPException(status_code=e.status_code, detail={"error_code": e.code, "message": e.message}) from e
    if result is None:
...
def decide_g07(run_id: str, request: G07DecisionRequest, actor: str = Depends(required_authenticated_actor), service: StagePreparationApplicationService = Depends(get_stage_service)):
    ...
    try:
        return service.decide_g07(run_id, request.stage_id, request.model_copy(update={"actor": actor}))
    except StageApplicationError as e:
        raise HTTPException(status_code=e.status_code, detail={"error_code": e.code, "message": e.message}) from e
