...
| AMFA-160 | AMFA-141 | S3-F02-I03 frontend | Command detail drawer | `commands.ts` exposes typed execute/list/get calls; `CommandExecutionPanel` is wired from the accepted policy decision with stable idempotency, stale handling, lifecycle detail, list rehydration, and artifact links. | IMPLEMENTED_NOT_RUNTIME_VERIFIED |
