...
export function useAuthoritativeRun(runId: string, initialState: AuthoritativeRunStateDto) {
  ...
  useEffect(() => {
    ...
    for (const type of [...AUTHORITATIVE_EVENT_TYPES, ...PARITY_BASELINE_EVENT_TYPES]) source.addEventListener(type, onEvent);
    const interval = window.setInterval(() => { void refresh(); }, 5000);
    void refresh();
    return () => {
      active = false;
      window.clearInterval(interval);
      for (const type of [...AUTHORITATIVE_EVENT_TYPES, ...PARITY_BASELINE_EVENT_TYPES]) source.removeEventListener(type, onEvent);
      source.close();
    };
  }, [refresh, runId]);
