...
export function FeasibilityPanel({ runId, initialState, connectionStatus, workflowEvents, refreshAuthoritativeState, headingLevel = 2 }: { runId: string; initialState: AuthoritativeRunStateDto; connectionStatus: string; artifacts: ArtifactRefDto[]; workflowEvents: Array<{ event_type: string; sequence: number }>; refreshAuthoritativeState?: () => Promise<unknown>; headingLevel?: PanelHeadingLevel }) {
  ...
  return <section className={styles.panel} aria-labelledby="feasibility-title">
    <div className={styles.previewHeader}><div><p className={styles.kicker}>S2-F05</p><Heading id="feasibility-title">Feasibility and G05</Heading><p className={styles.note}>Catalogue-owned compatibility truth; no local workflow advancement.</p></div><span className={styles.status}>{connectionLabel}</span></div>
    ...
    {feasibility ? <>
      ...
      <div className={styles.previewPanel}><Subheading>Immutable evidence</Subheading><ul className={styles.list}>{feasibility.artifact_ids.map((id) => <li key={id}><a className={styles.actionLink} href={feasibility.artifact_links[id] ?? `/api/v1/artifacts/${encodeURIComponent(id)}`} target="_blank" rel="noreferrer">{id}</a><code>{feasibility.artifact_checksums[id]}</code></li>)}</ul></div>
       <div className={styles.previewPanel}><Subheading>G05: {feasibility.gate_status}</Subheading>{approved ? <p role="status">G05 was accepted by the authoritative backend. This control does not locally advance the workflow.</p> : feasibility.gate_status === "blocked" ? <p role="alert">G05 is blocked until the feasibility evidence is renewed.</p> : <><label htmlFor="g05-decision">Decision</label><select id="g05-decision" value={decision} onChange={(event) => setDecision(event.target.value as G05Decision)}>{decisions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><label htmlFor="g05-comment">Review comment</label><textarea id="g05-comment" value={comment} onChange={(event) => setComment(event.target.value)} rows={3} placeholder="Optional rationale; required for approval with comment." /><button type="button" onClick={() => void submitDecision()} disabled={working || feasibility.gate_status !== "pending" || fingerprintMissing}>{working ? "Recording decision..." : "Record G05 decision"}</button></>}</div>
    </> : null}
  </section>;
}
