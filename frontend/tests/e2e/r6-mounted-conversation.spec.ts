...
async function ensureAssistantOpen(page: Page) {
  const panel = page.getByRole("heading", { name: "Migration Follow-up Assistant" });
  if (await panel.isVisible()) return;
  const open = page.getByRole("button", { name: "Open Assistant", exact: true });
  const expand = page.getByRole("button", { name: "Expand Assistant", exact: true });
  await expect.poll(async () => (await panel.isVisible()) || (await open.isVisible()) || (await expand.isVisible())).toBe(true);
  if (await panel.isVisible()) return;
  await (await expand.isVisible() ? expand : open).click();
}
