...
class BaselineInstallApplicationService:
    ...
    def accept(self, run_id: str, request: BaselineInstallRequest) -> BaselineInstallResponse:
        ...
        with self._lock: self._cancel_events[execution_id] = cancel_event
    ...
    def _execute(self, run_id, execution_id, request, sandbox, before, command, cancel_event):
        ...
        except BaselineInstallationError as error: return self._finalize_failure(run_id, execution_id, request, error.code)
        except OSError: return self._finalize_failure(run_id, execution_id, request, "BASELINE_INSTALL_ENVIRONMENT_BLOCKED", environment_blocker="PROCESS_START_FAILED")
        except Exception: return self._finalize_failure(run_id, execution_id, request, "BASELINE_INSTALL_EXECUTION_FAILED")
        finally:
            heartbeat_stop.set()
    ...
    def reconcile_orphans(self) -> int:
        ...
        with self._scope() as session:
            ...
            for record in records:
                ...
                record.blockers = ["WORKER_LOST_SAFE_TO_RERUN"] if safe_to_rerun else ["WORKSPACE_RECONSTRUCTION_REQUIRED"]
    ...
    def _output_chunk(self, run_id, execution_id, request, stream, chunk):
        with self._scope() as session:
            run = self._run(session, run_id)
            transition = self._transition(session, run, request, WorkflowEventType.COMMAND_OUTPUT_CHUNK, "baseline command output chunk", {"execution_id": execution_id, "stream": stream, "chunk": self._redact(chunk)}, expected_state_version=run.state_version)
    ...
    def _finalize(self, run_id, execution_id, request, before, result):
        with self._scope() as session:
            run = self._run(session, run_id); record = session.get(CommandExecutionModel, execution_id)
            artifacts = self._persist_artifacts(session, run, result, request)
            status = result.command.result.status; record.status = status.value; record.started_at = result.command.result.started_at; record.finished_at = result.command.result.finished_at; record.duration_ms = result.command.result.duration_ms; record.exit_code = result.command.result.exit_code; record.timed_out = result.command.timed_out; record.cancelled = result.command.cancelled; record.reconstruction_required = result.inspection.reconstruction_required; record.start_fingerprint = {"package_json": asdict(before[0]), "lockfile": asdict(before[1])}; record.end_fingerprint = {"package_json": asdict(result.inspection.package_json), "lockfile": asdict(result.inspection.lockfile)}; record.blockers = list(result.inspection.blockers); record.artifact_ids = artifacts; record.command_log_artifact_id = result.command.command_log_artifact.ref.artifact_id; record.stdout_artifact_id = result.command.stdout_artifact.ref.artifact_id if result.command.stdout_artifact else None; record.stderr_artifact_id = result.command.stderr_artifact.ref.artifact_id if result.command.stderr_artifact else None
            expected = record.state_version
            output = self._transition(session, run, request, WorkflowEventType.COMMAND_OUTPUT_AVAILABLE, "baseline npm ci output finalized", {"execution_id": execution_id, "artifact_count": len(artifacts)}, expected_state_version=expected); expected = output.next_state_version
            ...
            completed = self._transition(session, run, request, event_type, "baseline npm ci command completed", {"execution_id": execution_id, "status": record.status, "reconstruction_required": record.reconstruction_required}, expected_state_version=expected); record.state_version = completed.next_state_version; record.event_sequence = completed.event_sequence; session.flush(); return self._response(record)

    def _finalize_failure(self, run_id, execution_id, request, blocker, *, environment_blocker=None):
        with self._scope() as session:
            run = self._run(session, run_id); record = session.get(CommandExecutionModel, execution_id); record.status = CommandStatus.FAILED.value; record.finished_at = self._now(); record.blockers = [blocker]; record.environment_blocker = environment_blocker; completed = self._transition(session, run, request, WorkflowEventType.BASELINE_INSTALL_FAILED, "baseline npm ci command failed before completion", {"execution_id": execution_id, "blocker": blocker}, expected_state_version=record.state_version); record.state_version = completed.next_state_version; record.event_sequence = completed.event_sequence; session.flush(); return self._response(record)

    def _persist_artifacts(self, session, run, result, request):
        root = Path(run.artifact_root).resolve(); store = LocalFilesystemArtifactStore(root, fixed_run_root=root); artifacts = [result.command.command_log_artifact] + ([result.command.stdout_artifact] if result.command.stdout_artifact else []) + ([result.command.stderr_artifact] if result.command.stderr_artifact else [])
        payloads = {"01_baseline/npm-ci-command.json": {"command_id": "npm-ci-bootstrap", "executable": "npm", "arguments": ["ci"], "shell": False, "runtime_profile_id": request.runtime_profile_id, "runtime_checksum": request.runtime_checksum}, "01_baseline/dependency_tree_verification.json": asdict(result.inspection.dependency_tree) if result.inspection.dependency_tree else {"status": "not_run"}, "01_baseline/lockfile_post_install_verification.json": {"package_json": asdict(result.inspection.package_json), "lockfile": asdict(result.inspection.lockfile), "unchanged": not result.inspection.blockers}, "01_baseline/baseline_install_summary.json": {"status": result.inspection.status, "blockers": list(result.inspection.blockers), "reconstruction_required": result.inspection.reconstruction_required}}
        for relative_path, payload in payloads.items(): artifacts.append(store.write_text_artifact(run.id, relative_path, json.dumps(payload, indent=2, sort_keys=True, default=str), ArtifactType.JSON, created_by="baseline-install-application-service", created_at=self._now(), input_hashes={"request": request.idempotency_key, "baseline": request.runtime_checksum}))
        sandbox = Path((run.workspace_aliases or {})["BASELINE_SANDBOX"])
        for debug_path in sandbox.glob("npm-debug.log*"):
            try:
                content = self._redact(debug_path.read_text(encoding="utf-8", errors="replace"))
                artifacts.append(store.write_text_artifact(run.id, "01_baseline/" + debug_path.name, content, ArtifactType.TEXT_LOG, created_by="baseline-install-application-service", created_at=self._now(), input_hashes={"request": request.idempotency_key}))
            except OSError:
                continue
        ids=[]
        for artifact in artifacts:
            session.add(ArtifactMetadataModel(id=f"metadata-{artifact.ref.artifact_id}", run_id=run.id, stage_id=None, artifact_type=artifact.ref.artifact_type.value, relative_path=artifact.ref.relative_path, checksum=artifact.ref.checksum, created_at=artifact.ref.created_at)); ids.append(artifact.ref.artifact_id)
        return ids

    def _worker(self, run, runtime_profile_id):
        if self._worker_factory is not None: return self._worker_factory(run)
        baseline = Path((run.workspace_aliases or {})["BASELINE_SANDBOX"]).resolve(); root = Path(run.artifact_root).resolve(); store = LocalFilesystemArtifactStore(root, fixed_run_root=root); policy = CommandPolicy(sandbox_root=baseline, working_directory_aliases={"BASELINE_SANDBOX": baseline}, runtime_profiles=frozenset({runtime_profile_id}), network_profiles=frozenset({"approved-registries-only"})); return ExecutionWorker(policy, CommandLogWriter(store))
