...
import { cancelAuthoritativeRun, createAuthoritativeRun, getAuthoritativeRunState, startAuthoritativeRun } from "@/api/runs";
...
describe("authoritative run API client", () => {
  it("uses the versioned create, start, cancel, and state endpoints", async () => {
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ ...mutation, status: "CANCELLED", state_version: 5 }), { status: 200 }))
    ...
    await expect(cancelAuthoritativeRun("run-1", { expected_state_version: 4, idempotency_key: "cancel-1", actor: "operator" }, client)).resolves.toMatchObject({ status: "CANCELLED" });
    await expect(getAuthoritativeRunState("run-1", client)).resolves.toMatchObject({ preflight_id: "preflight-1" });
    expect(fetchMock.mock.calls.map(([url]) => url)).toEqual(["http://backend.test/api/v1/runs", "http://backend.test/api/v1/runs/run-1/start", "http://backend.test/api/v1/runs/run-1/cancel", "http://backend.test/api/v1/runs/run-1/state"]);
  });
});
