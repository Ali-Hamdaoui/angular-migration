...
export function CommandExecutionPanel({ runId, stateVersion, authorization, refreshAuthoritativeState }: { runId: string; stateVersion: number; authorization: CommandPolicyValidateResponseDto | null; refreshAuthoritativeState?: () => Promise<unknown> }) {
  ...
  return <section className={styles.panel} aria-label="Command executions">
    ...
    {selected ? <article className={styles.previewPanel} aria-label="Command execution detail"><h3>Execution detail</h3><div className={styles.dimensionGrid}><div><span>Execution ID</span><code>{selected.execution_id}</code></div><div><span>Run ID</span><code>{selected.run_id}</code></div><div><span>Status</span><strong>{label(selected.status)}</strong></div><div><span>Command template</span><code>{selected.template_id ?? selected.command_id}{selected.template_version ? ` v${selected.template_version}` : ""}</code></div><div><span>Stage</span><code>{selected.stage_id ?? "not supplied"}</code></div><div><span>Execution profile</span><code>{selected.execution_profile_id ?? "not supplied"}</code></div><div><span>Workspace alias</span><code>{selected.workspace_alias ?? "not supplied"}</code></div><div><span>State version</span><code>{selected.state_version}</code></div><div><span>Correlation ID</span><code>{selected.correlation_id ?? "not supplied"}</code></div><div><span>Exit code</span><code>{selected.exit_code ?? "not supplied"}</code></div><div><span>Failure</span><code>{selected.failure_code ?? "none"}</code></div><div><span>Duration</span><code>{selected.duration_ms === null ? "not supplied" : `${selected.duration_ms} ms`}</code></div></div>{selected.idempotent_replay ? <p className={styles.note}>Idempotent replay: this is the existing execution, not a new process.</p> : null}<h4>Finalized evidence</h4>{selected.artifact_ids.length === 0 ? <p className={styles.note}>Evidence is not available until the worker finalizes the execution.</p> : <ul className={styles.list}>{selected.artifact_ids.map((artifactId) => <li key={artifactId}><a className={styles.actionLink} href={`/api/v1/artifacts/${encodeURIComponent(artifactId)}`} target="_blank" rel="noreferrer">Open artifact {artifactId}</a></li>)}</ul>}</article> : null}
  </section>;
}
