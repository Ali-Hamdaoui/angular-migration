...
export type RunStatus = "CREATED" | "SOURCE_VALIDATION_RUNNING" | "SOURCE_VALIDATED" | "WORKSPACE_CLASSIFICATION_RUNNING" | "BASELINE_RUNNING" | "BASELINE_QUALIFIED" | "CLIENT_CONSTRAINTS_CAPTURED" | "ELIGIBILITY_RUNNING" | "ANALYSIS_RUNNING" | "WAITING_ANALYSIS_APPROVAL" | "PLANNING_RUNNING" | "WAITING_PLAN_APPROVAL" | "WAITING_STAGE_PREPARATION" | "WAITING_G07_APPROVAL" | "STAGE_CREATED" | "TOOLCHAIN_PROFILE_SELECTED" | "SANDBOX_READY" | "DEPENDENCY_AUDITED" | "TRANSFORMATION_RUNNING" | "STATIC_SYMBOL_CHECK_RUNNING" | "VALIDATION_RUNNING" | "REPAIR_RUNNING" | "WAITING_REPAIR_APPROVAL" | "REVIEW_READY" | "STAGE_COMMITTED" | "STAGE_ROLLED_BACK" | "REPORT_RUNNING" | "DELIVERY_RUNNING" | "PAUSE_REQUESTED" | "PAUSED" | "RESUMING" | "CANCEL_REQUESTED" | "CANCELLING" | "TIMED_OUT" | "WORKER_LOST" | "RECOVERY_RUNNING" | "ORPHANED" | "CLEANUP_RUNNING" | "CLEANUP_FAILED" | "RUNNING" | "WAITING" | "CANCELLED" | "COMPLETED" | "FAILED" | "DIAGNOSTIC_HOLD";
...
export type LogChunkResponseDto = { sequence: number; stream: string; text: string; redacted: boolean; truncated?: boolean; byte_count?: number; character_count?: number; created_at: string };
...
export type PlanningJobStatus = "queued_after_g04" | "resolving_feasibility" | "waiting_g05" | "generating_plan" | "running_planning_review" | "waiting_g06" | "waiting_retry" | "completed" | "completed_blocked" | "technical_failed" | string;
export type PlanningJobProjectionDto = { id: string; status: PlanningJobStatus; current_step: string; attempt: number; max_attempts: number; retryable?: boolean | null; next_attempt_at?: string | null; last_error_code?: string | null; last_error_message?: string | null; last_error_stage?: string | null; correlation_id?: string | null; updated_at: string };
export type PlanningCommandResponse = { job_id: string; status: string; current_step: string; correlation_id?: string | null };
export type AuthoritativeRunStateDto = { run_id: string; status: RunStatus; run_phase: RunPhase; phase_status: string; approval_status: ApprovalStatus; repair_status: RepairStatus; state_version: number; preflight_id: string; source_path: string; target_output_path: string; graph_thread_id: string; created_at: string; updated_at: string; workspace_aliases?: Record<string, string>; source_angular_exact?: string | null; catalogue_version?: string | null; registry_snapshot?: Record<string, unknown> | null; runtime_candidates?: Array<Record<string, unknown>>; planning_job?: PlanningJobProjectionDto | null; artifacts: ArtifactRefDto[]; workflow_events: WorkflowEventDto[] };
