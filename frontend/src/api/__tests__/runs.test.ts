...
import { cancelAuthoritativeRun, createAuthoritativeRun, getAuthoritativeRunState, retryAuthoritativeSourceIntake, startAuthoritativeRun } from "@/api/runs";
...
describe("authoritative run API client", () => {
  it("uses the versioned create, start, retry, cancel, and state endpoints", async () => {
    ...
    expect(fetchMock.mock.calls.map(([url]) => url)).toEqual(["http://backend.test/api/v1/runs", "http://backend.test/api/v1/runs/run-1/start", "http://backend.test/api/v1/runs/run-1/retry-source-intake", "http://backend.test/api/v1/runs/run-1/cancel", "http://backend.test/api/v1/runs/run-1/state"]);
  });
});
