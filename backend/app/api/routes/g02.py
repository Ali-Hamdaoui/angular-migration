...
@router.get("/{run_id}/approvals/{gate_id}", response_model=G02ReviewResponse)
def inspect_g02(run_id: str, gate_id: str, service: G02ApprovalApplicationService = Depends(get_g02_service)):
    result = service.get(run_id, gate_id)
    if result is None:
        raise HTTPException(status_code=404, detail="G02 approval package not found")
    return result


@router.post("/{run_id}/approvals/{gate_id}/decisions", response_model=G02ReviewResponse)
def decide_g02(run_id: str, gate_id: str, request: G02DecisionRequest, service: G02ApprovalApplicationService = Depends(get_g02_service)):
...
@router.post("/{run_id}/approvals/{gate_id}/package", response_model=G02ReviewResponse)
def initialize_g02(run_id: str, gate_id: str, request: G02PackageInitializationRequest, service: G02ApprovalApplicationService = Depends(get_g02_service)):
    if request.gate_id != gate_id:
        raise HTTPException(status_code=400, detail="gate_id in body does not match path")
    try:
