...
type CommandExecutionPanelProps = {
  runId: string;
  stateVersion: number;
  authorization: CommandPolicyValidateResponseDto | null;
  connectionStatus?: string;
  workflowEvents?: WorkflowEventDto[];
  refreshAuthoritativeState?: () => Promise<unknown>;
};

export function CommandExecutionPanel({ runId, stateVersion, authorization, connectionStatus, workflowEvents, refreshAuthoritativeState }: CommandExecutionPanelProps) {
  ...
  const commandEventSequence = useMemo(() => {
    const events = (workflowEvents ?? []).filter((event) => ["COMMAND_QUEUED", "COMMAND_STARTED", "COMMAND_SUCCEEDED", "COMMAND_FAILED"].includes(event.event_type));
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const result = await listCommandExecutions(runId);
      setExecutions(result.executions);
      setSelected((current) => result.executions.find((item) => item.execution_id === current?.execution_id) ?? result.executions[0] ?? null);
      setViewStatus("ready");
  ...
  useEffect(() => {
    if (connectionStatus === "reconnecting" || connectionStatus === "recovering") { setViewStatus("reconnecting"); return; }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      } catch { /* bounded refresh on the next lifecycle event */ }
    }, 1000);
  ...
  useEffect(() => {
    ...
    if (!selected || selected.artifact_ids.length === 0) {
      setArtifactMetadata({}); setArtifactMetadataStatus("idle");
      return () => { active = false; };
    }
    setArtifactMetadataStatus("loading");
    void Promise.all(selected.artifact_ids.map((artifactId) => getCommandArtifactById(artifactId)))
      .then((items) => { if (active) { setArtifactMetadata(Object.fromEntries(items.map((item) => [item.artifact.artifact_id, item.artifact]))); setArtifactMetadataStatus("ready"); } })
  ...
  return <section className={styles.panel} aria-label="Command executions">
    ...
    {selected ? <article className={styles.previewPanel} aria-label="Command execution detail">
      ...
      <div className={styles.dimensionGrid}>
        ...
        <div><span>Created</span><time dateTime={selected.created_at ?? undefined}>{timestamp(selected.created_at)}</time></div><div><span>Started</span><time dateTime={selected.started_at ?? undefined}>{timestamp(selected.started_at)}</time></div><div><span>Completed</span><time dateTime={selected.completed_at ?? undefined}>{timestamp(selected.completed_at)}</time></div><div><span>Duration</span><code>{selected.duration_ms === null ? "not supplied" : `${selected.duration_ms} ms`}</code></div><div><span>Failure</span><code>{selected.failure_code ?? "none"}</code></div>
      </div>
      <div><h4>Exact argv</h4><ol>{(selected.arguments ?? []).map((argument, index) => <li key={`${index}-${argument}`}><code>{argument}</code></li>)}</ol></div>
