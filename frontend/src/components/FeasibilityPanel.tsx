...
const decisions: Array<{ value: G05Decision; label: string }> = [
  { value: "approve", label: "Approve migration readiness" },
  { value: "approve_with_comment", label: "Approve with comment" },
  { value: "request_modification", label: "Request readiness changes" },
  { value: "reject", label: "Reject migration readiness" },
];
...
export function FeasibilityPanel({ runId, initialState, connectionStatus, workflowEvents, refreshAuthoritativeState, headingLevel = 2 }: { runId: string; initialState: AuthoritativeRunStateDto; connectionStatus: string; artifacts: ArtifactRefDto[]; workflowEvents: Array<{ event_type: string; sequence: number }>; refreshAuthoritativeState?: () => Promise<unknown>; headingLevel?: PanelHeadingLevel }) {
  ...
  async function submitDecision() {
    ...
    try {
      ...
      if (reason instanceof ApiClientError && reason.status === 409) { setStale(true); await refresh(); }
      else setError(`Migration readiness decision failed. Correlation ID: ${reason instanceof ApiClientError ? correlationFrom(reason) : "unavailable"}`);
    } finally { setWorking(false); }
  }

  const approved = feasibility?.gate_status === "approved";
  const connectionLabel = connectionStatus === "reconnecting" ? "Connection lost. Reconnecting..." : connectionStatus === "recovering" ? "Refreshing authoritative feasibility state..." : feasibility?.status === "in_progress" ? "Running" : feasibility?.status === "blocked" ? "Blocked" : feasibility?.status === "completed" ? "Complete" : working ? "Working" : "Loading";
  return <section className={styles.panel} aria-labelledby="feasibility-title">
    <div className={styles.previewHeader}><div><p className={styles.kicker}>Migration readiness</p><Heading id="feasibility-title">Migration readiness</Heading><p className={styles.note}>Catalogue-owned compatibility truth; no local workflow advancement.</p></div><span className={styles.status}>{connectionLabel}</span></div>
    {loading ? <p role="status">Loading authoritative feasibility...</p> : null}
    {error ? <p role="alert">{error}</p> : null}
    {stale ? <p role="alert">The migration-readiness state is stale. The authoritative snapshot was reloaded; review the current package before retrying.</p> : null}
    ...
    {feasibility && needsFingerprintRebind ? <div role="alert"><p>This feasibility package is not bound to the approved baseline workspace. Regenerate it before approving migration readiness or generating a migration plan.</p><button type="button" onClick={() => void resolve("feasibility-rebind")} disabled={working}>{working ? "Regenerating feasibility..." : "Regenerate fingerprint-bound feasibility"}</button></div> : null}
    {feasibility?.status === "blocked" ? <div role="alert"><p>Feasibility is blocked; migration readiness cannot approve this route.</p><ul className={styles.list}>{feasibility.blockers.map((item) => <li key={item}><code>{item}</code></li>)}</ul></div> : null}
    {feasibility ? <>
      ...
      <Subheading>Major-stage ladder</Subheading><ol className={styles.list}>{feasibility.route.map((stage) => <li key={stage.stage_id}><strong>{stage.source_family} → {stage.target_family}</strong> <span className={styles.status}>{stage.support_level}</span>{stage.target_angular_exact ? <span> · Angular {stage.target_angular_exact} / CLI {stage.target_cli_exact}</span> : null}</li>)}</ol>
      {feasibility.warnings.length ? <div><Subheading>Warnings</Subheading><ul className={styles.list}>{feasibility.warnings.map((item) => <li key={item}>{item}</li>)}</ul></div> : null}
      <div className={styles.previewPanel}><Subheading>Candidate runtimes</Subheading>{(feasibility.runtime_candidates ?? []).length ? <ul className={styles.list}>{(feasibility.runtime_candidates ?? []).map((candidate) => <li key={String(candidate.profile_id)}><strong>{String(candidate.profile_id)}</strong> · {String(candidate.operating_system)}/{String(candidate.architecture)} · Node {String(candidate.node_exact)} · npm {String(candidate.npm_exact)} · npx {String(candidate.npx_exact)}</li>)}</ul> : <p className={styles.note}>No runtime candidates were supplied by the authoritative backend.</p>}<Subheading>Exact Stage 1 profile</Subheading>{feasibility.selected_profile ? <div className={styles.dimensionGrid}><div><span>Angular / CLI</span><strong>{feasibility.selected_profile.angular_exact} / {feasibility.selected_profile.angular_cli_exact}</strong></div><div><span>Node / npm / npx</span><strong>{feasibility.selected_profile.node_exact} / {feasibility.selected_profile.npm_exact} / {feasibility.selected_profile.npx_exact}</strong></div><div><span>Profile</span><strong>{feasibility.selected_profile.profile_id}</strong></div><div><span>Checksum</span><code>{feasibility.selected_profile.checksum}</code></div></div> : <p className={styles.note}>No exact Stage 1 profile was resolved.</p>}</div>
      <div className={styles.previewPanel}><Subheading>Immutable evidence</Subheading><ul className={styles.list}>{feasibility.artifact_ids.map((id) => <li key={id}><a className={styles.actionLink} href={feasibility.artifact_links[id] ?? `/api/v1/artifacts/${encodeURIComponent(id)}`} target="_blank" rel="noreferrer">{id}</a><code>{feasibility.artifact_checksums[id]}</code></li>)}</ul></div>
       <div className={styles.previewPanel}><Subheading>Migration readiness approval</Subheading><p className={styles.note}>Current decision: {feasibility.gate_status === "approved" ? "Approved" : feasibility.gate_status === "approved_with_comment" ? "Approved with comment" : feasibility.gate_status === "request_modification" ? "Changes requested" : feasibility.gate_status === "rejected" ? "Rejected" : feasibility.gate_status === "blocked" ? "Blocked" : "Awaiting approval"}</p>{approved ? <p role="status">Migration readiness was approved by the authoritative backend. This control does not locally advance the workflow.</p> : feasibility.gate_status === "blocked" ? <p role="alert">Migration readiness approval is blocked until the feasibility evidence is renewed.</p> : <><label htmlFor="g05-decision">Decision</label><select id="g05-decision" value={decision} onChange={(event) => setDecision(event.target.value as G05Decision)}>{decisions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><label htmlFor="g05-comment">Review comment</label><textarea id="g05-comment" value={comment} onChange={(event) => setComment(event.target.value)} rows={3} placeholder="Optional rationale; required for approval with comment." /><button type="button" onClick={() => void submitDecision()} disabled={working || feasibility.gate_status !== "pending" || fingerprintMissing}>{working ? "Recording decision..." : "Record migration readiness decision"}</button></>}</div>
    </> : null}
  </section>;
}
