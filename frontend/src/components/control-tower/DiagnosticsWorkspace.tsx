...
class PanelBoundary extends Component<{ children: ReactNode; title: string }, { failed: boolean }> {
  ...
  render() {
    return this.state.failed
      ? <section className={styles.card} role="alert"><h3>{this.props.title} is temporarily unavailable</h3><p className={styles.note}>The rest of Diagnostics remains available. Refresh to retry this panel.</p></section>
      : this.props.children;
  }
}
...
export function DiagnosticsWorkspace({
  ...
  return <section className={styles.workspace} aria-labelledby="diagnostics-workspace-title">
    ...
    <section className={styles.card} aria-label="Migration timing" aria-labelledby="diagnostics-timing-title">
      <h3 id="diagnostics-timing-title">Migration timing</h3>
      <PanelBoundary title="Migration timing"><MigrationTimingPanel runId={runId} refreshKey={run.workflow_events.at(-1)?.sequence ?? 0} /></PanelBoundary>
    </section>
