...
export function ControlTowerShell({ run, runId, connectionStatus }: { run: MigrationRun; runId?: string; connectionStatus?: ConnectionStatus }) {
  ...
  return <main className={styles.shell}><div className={styles.content}>
    <nav className={styles.legacyNav} aria-label="Run sections">
      <button type="button" className={styles.legacyNavActive}>Overview</button>
      <button type="button">Pipeline</button>
      <button type="button">Evidence</button>
      <button type="button">Diagnostics</button>
    </nav>
    <RunHeader run={run} /><WorkflowTimeline run={run} /><StageCards run={run} /><div className={styles.dashboardGrid}><div className={styles.primaryColumn}><AgentActivityPanel run={run} />{runId ? <BaselineValidationPanel runId={runId} stateVersion={stateVersion} connectionStatus={matrixConnection} /> : null}{runId ? <BaselineQualificationPanel runId={runId} stateVersion={stateVersion} /> : null}<ArtifactPanel run={run} /><ReportPanel run={run} /></div><aside className={styles.secondaryColumn}><ValidationGatePanel run={run} /><ApprovalPanel run={run} /><DiagnosticsPanel run={run} /><LlmUsagePanel run={run} /></aside></div></div></main>;
}
